...
from .value import KnownValue, UnboundMethodValue, UNINITIALIZED_VALUE
...
class VarnameGenerator(object):
    ...
    def _ensure_unique(self, varname):
...
class YieldChecker(object):
    def __init__(self, visitor):
        self.variables_from_yield_result = {}
        self.visitor = visitor
        self.in_yield_result_assignment = False
        self.in_non_async_yield = False
        self.last_yield_in_aug_assign = False
        self.previous_yield = None
        self.statement_for_previous_yield = None
    ...
    def record_call(self, value, node):
        if (
    ...
    def _maybe_show_unnecessary_yield_error(self, node, current_statement):
    ...
    def show_unnecessary_yield_error(self, unused, node, current_statement):
        if not unused:
    ...
    def _lines_of_node(self, yield_node):
    ...
    def _create_replacement_for_yield_nodes(self, second_node, second_parent):
        ...
        if first_yield.is_assign_or_expr() and second_yield.is_assign_or_expr():
            ...
            to_add = new_assign_lines + between_lines + replace_yield.lines_to_add
    ...
    def _merge_assign_nodes(self, first_yield, second_yield):
    ...
    def _indentation_of_node(self, node):
    ...
    def generate_varname_from_node(self, node):
